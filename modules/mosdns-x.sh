...
setup_system_dns() {
    echo_info "配置系统 DNS 为 127.0.0.1..."
    
    # 备份原始 resolv.conf
    if [[ -f /etc/resolv.conf && ! -f /etc/resolv.conf.bak ]]; then
        cp /etc/resolv.conf /etc/resolv.conf.bak
        echo_info "已备份原始 DNS 配置到 /etc/resolv.conf.bak"
    fi
    
    # 检查是否使用 systemd-resolved
    if systemctl is-active --quiet systemd-resolved 2>/dev/null; then
        echo_info "检测到 systemd-resolved，正在配置..."
        
        # 创建 resolved 配置目录
        mkdir -p /etc/systemd/resolved.conf.d
        
        # 配置 systemd-resolved 使用本地 DNS
...
EOF
        ...
        # 更新 /etc/resolv.conf 符号链接
        if [[ -L /etc/resolv.conf ]]; then
            rm -f /etc/resolv.conf
        fi
        ln -sf /run/systemd/resolve/resolv.conf /etc/resolv.conf
        
        echo_success "systemd-resolved 配置完成。"
    else
        echo_info "配置传统 resolv.conf..."
...
# MosDNS-X DNS Configuration
# Generated by mosdns-x installation script
# Original configuration backed up to /etc/resolv.conf.bak

nameserver 127.0.0.1
options edns0 trust-ad
EOF
        
        # 设置 immutable 属性防止被覆盖
        chattr +i /etc/resolv.conf 2>/dev/null || true
        
        echo_success "DNS 配置已更新为 127.0.0.1"
    fi
    
    # 验证 DNS 配置
    echo_info "验证 DNS 配置..."
    if grep -q "127.0.0.1" /etc/resolv.conf; then
        echo_success "系统 DNS 已成功设置为 127.0.0.1"
    else
        echo_warn "DNS 配置可能未生效，请手动检查。"
    fi
}
...
show_summary() {
    ...
    echo "  DNS 服务监听地址: 127.0.0.1:53 (TCP/UDP)"
    echo "  系统 DNS: 127.0.0.1 (已接管系统 DNS)"
    echo
    echo "  上游 DNS 服务器:"
    echo "    - 1.1.1.1 (Cloudflare)"
    echo "    - 8.8.8.8 (Google)"
    echo "    - tls://unfiltered.adguard-dns.com (AdGuard DNS over TLS)"
    echo
    echo "  常用命令:"
    ...
    echo "    - 测试DNS:  dig @127.0.0.1 google.com"
